...
export default class FirebaseCartsContainer{
    constructor(uri){
    ...
    async createCart(){
        try {
            ...
            await this.createAndPushCart(idCart)
            return idCart
        } catch (err){
    ...
    async _validateExistCart(idCart){
        try {
            const carts = await this._getCartsFromFile();
            const searchedCart = carts.find( cart => cart.id == idCart);
            if(!searchedCart){
                const error = new Error(`The cart with id: '${idCart}' doesn't exist`);
                throw error; 
    ...
    async postProductToCart(idCart, data){
        try {
            ...
            await collection.doc(cart._id).update({id:cart.id,products:cart.products});

            const doc = await collection.doc(cart._id).get();
    ...
    async deleteProductFromCart(idCart,idProduct){
        try {
            ...
            await collection.doc(carts[indexCart]._id).update({id:carts[indexCart].id, products:carts[indexCart].products});

            const doc = await collection.doc(carts[indexCart]._id).get();
    ...
    async deleteCart(idCart){
        try {
            const cart = await this._validateExistCart(idCart);
            ...
            await collection.doc(cart._id).delete();
